...
`Jdbi` itself does not provide connection pooling or other <<High Availability>>
...
released.  In the example above, the two `insertFullContact` operations take
separate `Connection` objects from your database connection pool.
...
for qualifiers on constructor parameters, getter, setters, and setter parameters of Kotlin class.
...
with Spring 5.1.8 (used by Spring Boot 2.1.4) as well.
...
We've used link:https://brettwooldridge.github.io/HikariCP/[HikariCP^] in combination
